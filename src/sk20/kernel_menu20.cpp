...
boolean CKernelMenu::Initialize( void )
{
	...
	initGlobalMemPool(1024 * 1024 * 20);
	...
	u32 size = 0;
	logger->Write("VIC20", LogNotice, "%s: - 5", __FUNCTION__);

	readFile( logger, (char*)DRIVE, (char*)FILENAME_SPLASH_HELP, helpscreenRAW, &size );
	logger->Write("VIC20", LogNotice, "%s: - 6", __FUNCTION__);
	...
	logger->Write("VIC20", LogNotice, "%s: - 12", __FUNCTION__);

	renderC64();
	convertScreenToBitmap( framebuffer );
	logger->Write("VIC20", LogNotice, "%s: - 13", __FUNCTION__);

	latchSetClearImm( 0, LATCH_LED0to1 );
	logger->Write("VIC20", LogNotice, "%s: - 14", __FUNCTION__);
...
int skvic20_main( CKernel &main_kernel )
{
	CKernelMenu kernel(main_kernel.get_isys(), main_kernel.get_timer(), 
					   main_kernel.get_logger(), main_kernel.get_scrdevice());
	logger->Write("VIC20", LogNotice, "%s: - 1", __FUNCTION__);
	kernel.Initialize();
